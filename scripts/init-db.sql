...
CREATE TABLE IF NOT EXISTS transactions.sources (
...
CREATE TABLE IF NOT EXISTS transactions.categories (
    ...
    name VARCHAR(255) NOT NULL UNIQUE,
    description TEXT,
    subcategory_of UUID REFERENCES transactions.categories(id) ON DELETE SET NULL,
    keywords TEXT[],
    version INT NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS transactions.transactions (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    source_id UUID REFERENCES transactions.sources(id) ON DELETE CASCADE,
